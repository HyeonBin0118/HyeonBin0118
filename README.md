...
다중 사용자 동시 접속 환경에서 병목을 직접 측정하고, 캐싱 전략으로 개선하는 부하 테스트 사이클을 구현했다.

- **[ai-personal-assistant](https://github.com/HyeonBin0118/ai-personal-assistant)**
  `FastAPI` `PostgreSQL` `Redis` `APScheduler` `Locust` `GPT-4o-mini` `Docker`
  · 실제 LLM 포함 50명 동시 부하 측정 → `/input` p50 1,500ms 병목 확인
  · 완전 일치 캐싱 히트율 10% → 임베딩 유사도 캐싱(text-embedding-3-small)으로 개선 진행 중
  · 랜덤 문장 생성기로 현실적인 부하 시나리오 직접 설계 (고정 문장 10개의 한계를 측정으로 발견)
  · JWT 인증 + 계정별 데이터 격리 + 백그라운드 스케줄러 기반 알림
