# Hi, I'm HyeonBin 👋
AI/ML 프로젝트를 직접 설계하고 끝까지 구현하는 것을 좋아합니다.  
단순히 코드를 작성하는 것보다 왜 이 선택을 했는지 데이터로 설명할 수 있는 개발을 지향합니다.
...
- **[job-agent-v3](https://github.com/HyeonBin0118/job-agent-v3)** — v2 개선, 텍스트 전처리 강화 + 정량 평가 + 자소서 품질 자동 평가 + 다중 공고 비교  
  `LangGraph` `GPT-4o-mini` `Streamlit` · [🚀 Demo](https://job-agent-v3-yzhg5h4dlyceriez67kudz.streamlit.app)
...
## 🛠️ Tech Stack
`Python` `PyTorch` `LangChain` `LangGraph` `ChromaDB` `OpenAI API` `Cohere` `FastAPI` `Streamlit` `YOLOv5` `OpenCV`
